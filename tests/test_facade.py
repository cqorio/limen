...
def test_config_dict_with_string_modes():
    limen = Limen(MemoryStore(), config={"sequence_anomaly": "enforce", "account_budget": "off"})
...
def test_observe_feeds_state_and_evaluate_enforces_next():
    limen = Limen(MemoryStore(), config={"enumeration": Mode.ENFORCE})
    ip = "9.9.9.9"
    for _ in range(25):  # default enumeration limit is 20
        limen.observe(RequestContext(method="GET", path="/api/reports/x", status=404, ip=ip))
